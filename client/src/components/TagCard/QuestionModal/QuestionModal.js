...
class QuestionModal extends Component {
  ...
  addTag = async () => {
    ...
    if (add) {
      ...
      try {
        const res = await axios.post("https://floating-eyrie-36103.herokuapp.com/api/tags", body, {
          headers: {
  ...
  getTags = async () => {
    try {
      ...
      const res = await axios.post(
        "https://floating-eyrie-36103.herokuapp.com/api/problem/problem/" + this.props.problem._id,
        body
      );
  ...
  render() {
    ...
    return (
      <Modal
        ...
        trigger={<a style={{fontWeight: this.props.bold ? "bold" : "normal"}}>{this.props.title}</a>}
      >
